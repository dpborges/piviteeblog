...
// import '../../styles/settings.modulescss'
...
/* 
    ...
    If you do not want to display it, use config prop described below.
...
export class DropDownMenu extends React.Component {
    ...
    // handleClickOutside = () => {
    //     console.log("*********   C L I C K E D   O U T    S I D E   ********")
    //     this.setState({
    //           listOpen: false     
    //     })
    // }
    ...
    render() {
        ...
        let { listOpen, displayCount, displayHeader } = this.state;

        /* set up default classes */
        let headerClass = "drop-down-header";
        let ddListClass = "drop-down-list";
        ...
        if (altHeaderClass) { headerClass = altHeaderClass}
        if (altDDListClass) { ddListClass = altDDListClass}
        ...
        <div className="drop-down-container">
            <div className={`${headerClass}`} onClick={() => this.toggleList()}>
                {displayCount === "no" &&  <div className="drop-down-title ss-page-header-navlink">{title}</div>}
                {displayCount === "selected" && <div className="drop-down-title ss-page-header-navlink">{title}&nbsp;{numSelected === 0 ? '' : `(${numSelected})`}</div>}
                {displayCount === "all" &&  <div className="drop-down-title ss-page-header-navlink">{title}&nbsp;{list.length === 0 ? '' : `(${list.length})`}</div>}
                <div>
                    {listOpen
                        ? <AngleUpIcon />
                        : <AngleDownIcon />
            ...
            {listOpen &&  <ul className={`${ddListClass}`} >
                ...
                {list.map((item) => (
                <li className={styles.dropDownListItem} key={item.key} 
